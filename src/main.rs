...
use rsync_tree::{Tree, TreeBuildError, build_tree_from_rsync_output_string};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load tree from file
    let tree = if let Some(ref load_path) = args.load_tree {
        info!("Loading tree from file: {}", load_path.display());
        let tree = Tree::load_from_file(load_path).map_err(|e| {
            error!("Failed to load tree from {}: {}", load_path.display(), e);
            e
        })?;
        info!("Tree loaded successfully from {}", load_path.display());
        tree
    } else {
        ...
        let tree =
            match build_tree_from_rsync_output_string(&rsync_output, &base_path, args.show_sizes) {
                Ok(tree) => tree,
        ...
        if let Some(ref save_path) = args.save_tree {
            ...
            tree.save_to_file(save_path).map_err(|e| {
                error!("Failed to save tree to {}: {}", save_path.display(), e);
                e
            })?;
            info!("Tree saved successfully to {}", save_path.display());
        }
